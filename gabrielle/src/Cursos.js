...
function Cursos({ abrirModal, usuarioLogado,  }) {
    ...
    const handleInscricao = async (cursoId) => {
        ...
        try {
            ...
            // Após inscrição ou desinscrição, atualizar a contagem de inscrições
            const responseCount = await axios.get(`http://localhost:3001/api/contarInscricoes/${cursoId}`);
            setInscricaoCount((prevState) => ({
                ...prevState,
                [cursoId]: responseCount.data.count, // Atualiza a contagem de inscrições
            }));
